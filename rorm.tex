...
$x$ and $y$ are in sometimes inverse causal relation (denoted as $y\rightharpoonup^{-1}x$) iff: $\forall\sigma_{y}\in S_{y}$, $x$ and $y$ are neither in always inverse causal relation nor never in inverse causal relation.
\end{definition}
...
In Section \ref{subsec:causalAndInverseCausal} and Section \ref{subsec:concurrent}, we give the definitions of extended refined ordering relations with uncertainty between events of the complete prefix unfolding. However, more useful information are the extended relations between transitions on the WF-net. In this section, we focus on the computation of our relations on a WF-net based on its unfolding.
