...
\begin{definition}[Process run/Run, adapted from \cite{polyvyanyy20144c}]\label{def:run}
...
Obviously, we can generate the set of process runs of a WF-net from its CPU by ``unfolding'' the net after the cut-off events. In other words, CPU is the minimal but complete expression of all the runs in a model.
...
Let $\Theta$ be the set containing all the runs generated from $U$, $\Theta_{x}$ be the set of the runs containing event $x$, i.e, $\Theta_{x}=\{\beta\in\Theta|x\in\beta\}$ and $FS(\beta)$ be the set of all the execution sequences of run $\beta$. Our extended causal relations are defined using the notion of $\Theta_{x}$ and $FS$.
...
Events $x$ and $y$ are in:
	\begin{itemize}
		...
		\item[-] \textbf{never causal relation} (denoted as $x\overset{\text{\tiny{N}}}{\rightarrow} y$) iff: $\forall\beta_{x}\in\Theta_{x}$, $\forall\sigma\in FS(\beta_{x})$: when $x$ executes, $y$ mustn't be executed after it.
		\item[-] \textbf{sometimes causal relation} (denoted as $x\overset{\text{\tiny{S}}}{\rightarrow} y$) iff: neither $x\twoheadrightarrow y$ nor $x\overset{\text{\tiny{N}}}{\rightarrow} y$.
	\end{itemize}
...
\textbf{Always Inverse Causal} ($\overset{\text{\tiny{A}}}{\leftarrow}$), \textbf{Sometimes Inverse Causal} ($\overset{\text{\tiny{S}}}{\leftarrow}$) and \textbf{Never Inverse Causal} ($\overset{\text{\tiny{N}}}{\leftarrow}$) can be defined in a similar way. The relations above are collectively called extended refined inverse causal relation with uncertainty (denoted as $\leftarrow$). Also, we further refine $\overset{\text{\tiny{A}}}{\leftarrow}$ and $\overset{\text{\tiny{S}}}{\leftarrow}$ into four relations, i.e., $\overset{\text{\tiny{DA}}}{\leftarrow}$, $\overset{\text{\tiny{IA}}}{\leftarrow}$, $\overset{\text{\tiny{DS}}}{\leftarrow}$ and $\overset{\text{\tiny{IS}}}{\leftarrow}$.
...
Transitions $A$ and $B$ are in: 
	\begin{itemize}
		...
		\item[-] \textbf{never causal relation} (denoted as $A\overset{\text{\tiny{N}}}{\rightarrow} B$) iff: $\forall a\in Corr_{E}(A), \forall b\in Corr_{E}(B), a\overset{\text{\tiny{N}}}{\rightarrow} b$.
...
Transitions $B$ and $A$ are in: 
	\begin{itemize}
		...
		\item[-] \textbf{never inverse causal relation} (denoted as $B\overset{\text{\tiny{N}}}{\leftarrow} A$) iff: $\forall b\in Corr_{E}(B), \forall a\in Corr_{E}(A), b\overset{\text{\tiny{N}}}{\leftarrow} a$.
...
As we can see from the result, the time costs of ExRORU is reasonable considering that ExRORU contains many details about relations between tasks. To our best knowledge, ExRORU is the only existing algorithm that can handle many complex structures such as silent transitions, non-free-choice constructs and multi-relation, which shows its outstanding effectiveness.
...
The methods in our approach and RORU are close in spirit. Their approach is based on two fundamental relations, causal and concurrent, each of which is refined into four sub types. All the relations in RORU are bi-directional with uncertainty. However, we have found that RORU is not capable to uniquely describe all processes, as we showed in Section \ref{sec:introduction}. The most severe problem in RORU is that it cannot deal with cyclic models. We check the datasets in Section \ref{subsec:evaluation} and find out that around 20\% of DG, 60\% of TC and 5\% of SAP are cyclic models. (The reason that cyclic models in SAP are rare is that models in SAP are mainly real-life reference models and lack redo structure.)

As an improvement, we introduce the \textit{inverse causal} relation so that every relation in Table \ref{tab:relations} can be uni-directional, and we use the \textit{Direct and Indirect Causal} relation to strengthen our ability of describing a process. With the technique of ``unfolding'', ExRORU can work on cyclic models. Table \ref{tab:corr} shows the correspondences between the relations of each methods. We use ``never'' to describe the relation between two transitions that are in conflict (i.e., $x\#y$). RORU cannot differentiate the two models in both of which transitions are all in conflict with each other while we can.
\\
\begin{table}[htbp]
\centering
\caption{Correspondences between the relations of RORU and ExRORU\label{tab:corr}}
	\begin{tabular}{|c|c|c|c|c|c|c|c|c|c|c|}\hline
		RORU & $x\twoheadrightarrow y$ & $x\rightharpoonup y$ & $x\mapsto y$ & $x\rightleftharpoons y$ & & $x\equiv y$ & $x\Downarrow y$ & $x\Uparrow y$ & $x\Updownarrow y$ & \\ \hline
		ExRORU & \tabincell{c}{$x\overset{\text{\tiny{A}}}{\rightarrow} y$\\$x\overset{\text{\tiny{A}}}{\leftarrow} y$} & \tabincell{c}{$x\overset{\text{\tiny{S}}}{\rightarrow} y$\\$x\overset{\text{\tiny{A}}}{\leftarrow} y$} & \tabincell{c}{$x\overset{\text{\tiny{A}}}{\rightarrow} y$\\$x\overset{\text{\tiny{S}}}{\leftarrow} y$} & \tabincell{c}{$x\overset{\text{\tiny{S}}}{\rightarrow} y$\\$x\overset{\text{\tiny{S}}}{\leftarrow} y$} & \tabincell{c}{$x\overset{\text{\tiny{N}}}{\rightarrow} y$\\$x\overset{\text{\tiny{N}}}{\leftarrow} y$} & \tabincell{c}{$x\Updownarrow y$\\$y\Updownarrow x$} & \tabincell{c}{$x\Uparrow y$\\$y\Updownarrow x$} & \tabincell{c}{$x\Updownarrow y$\\$y\Uparrow x$} & \tabincell{c}{$x\Uparrow y$\\$y\Uparrow x$} & \tabincell{c}{$x\nparallel y$\\$y\nparallel x$}\\ \hline
	\end{tabular}
\end{table}

Table \ref{tab:summary} gives a summary comparison between RORU and ExRORU. As proved before, ExRORU guarantees the uniqueness property while RORU fails. ExRORU has more relations so that it has a higher granularity level for uniqueness and guarantees that some relation always exists between any pair of transitions.

\begin{table}[htbp]
\centering
\caption{A summary of qualitative comparison of RORU vs. ExRORU\label{tab:summary}}
	\begin{tabular}{|l|l|l|}\hline
		\diagbox[width=15em,trim=l]{Attribute}{Method} & RORU & ExRORU\\ \hline
		Uniqueness & Fails & Guarantees\\ \hline
		Fundamental relations & 2 & 3\\ \hline
		Total relations & 8 & 13\\ \hline
		...
		\tabincell{l}{Relation between any pairs of\\transitions} & No & Yes\\ \hline
		Adjacency between transitions & No & Yes\\ \hline
		Dealing with cyclic models & Cannot & Can\\ \hline
	\end{tabular}
\end{table}
