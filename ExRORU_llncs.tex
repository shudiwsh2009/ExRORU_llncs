...
In this section we first compare ExRORU and RORU in \cite{jin2014computing}.

The methods in our approach and RORU are close in spirit. Their approach is based on two fundamental relations, causal and concurrent, each of which is refined into four sub types. All the relations in RORU are bi-directional represented by the notion of uncertainty. However, we have found that RORU are not capable to uniquely describe all processes, as we showed in Section \ref{sec:introduction}. As an improvement, we introduce the \textit{inverse causal} relation so that every relation in Table \ref{tab:relations} can be uni-directional, and we use the \texit{Direct and Indirect Causal} relation to strengthen our ability of describing a process.
