...
The methods in our approach and \cite{jin2014computing} (we use RORU to represent it) are close in spirit. Their approach is based on two fundamental relations, causal and concurrent, each of which is refined into four sub types. All the relations in RORU are bi-directional with uncertainty. However, we have found that RORU is not capable to uniquely describe all processes, as we showed in Section \ref{sec:introduction}. As an improvement, we introduce the \textit{inverse causal} relation so that every relation in Table \ref{tab:relations} can be uni-directional, and we use the \textit{Direct and Indirect Causal} relation to strengthen our ability of describing a process.

The most severe problem in RORU is that it cannot deal with cyclic models. We check the datasets in Section \ref{subsec:efficiency} and find out that around 20\%(TBD) of the real-life models are cyclic.
