...
If there is any difference in the trace-semantics behavior between process models $P$ and $Q$, then the ExRORU matrices of them are different.
\end{theorem}

\begin{proof}
Given any process model $P$, if we make any primitive change to it such as adding, deleting or restructuring a visible or silent transition to produce a new model $Q$, the fingerprints of $P$ and $Q$ will always be different. We'll discuss three types of changes as follows:\\
\textbf{Restructuring an transition}. It will certainly change the existing relationships between some transitions, which immediately influence the matrix.\\
\textbf{Adding/deleting a visible transition}. No matter how a new visible transition is added to model $P$, it has a basic ordering relation with an existing transition, which obviously makes the ExRORU matrix of the new model $Q$ different from that of the original model $P$. By induction, any new model derived from an existing model by adding a visible transition in successive steps will have a different ExRORU matrix. The proof of deleting a visible transition is similar.\\
\textbf{Adding/deleting a silent transition}. The \textit{Direct and Indirect Causal} relation will be influenced if a silent transition is included in sequential, parallel or exclusive relation with an existing transition, thus, the ExRORU matrix will change. Also by induction, successive steps of adding a silent transition will produce a new model with different ExRORU matrix from the original one. The proof of deleting a silent transition is similar.\qed
\end{proof}
...
The methods in our approach and \cite{jin2014computing} (we use RORU to represent it) are close in spirit. Their approach is based on two fundamental relations, causal and concurrent, each of which is refined into four sub types. All the relations in RORU are bi-directional with uncertainty. However, we have found that RORU is not capable to uniquely describe all processes, as we showed in Section \ref{sec:introduction}. The most severe problem in RORU is that it cannot deal with cyclic models. We check the datasets in Section \ref{subsec:efficiency} and find out that around 20\% of DG, 60\% of TC and 5\% of SAP are cyclic models. (The reason that cyclic models in SAP are rare is that models in SAP are mainly real-life reference models and lack redo structure.)
...
\caption{Correspondences between the relations of RORU and ExRORU\label{tab:corr}}
	\begin{tabular}{|c|c|c|c|c|c|c|c|c|c|c|}\hline
		...
		ExRORU & \tabincell{c}{$x\twoheadrightarrow y$\\$x\twoheadleftarrow y$} & \tabincell{c}{$x\rightharpoonup y$\\$x\twoheadleftarrow y$} & \tabincell{c}{$x\twoheadrightarrow y$\\$x\leftharpoonup y$} & \tabincell{c}{$x\rightharpoonup y$\\$x\leftharpoonup y$} & \tabincell{c}{$x\nrightarrow y$\\$x\nleftarrow y$} & \tabincell{c}{$x\Updownarrow y$\\$y\Updownarrow x$} & \tabincell{c}{$x\Uparrow y$\\$y\Updownarrow x$} & \tabincell{c}{$x\Updownarrow y$\\$y\Uparrow x$} & \tabincell{c}{$x\Uparrow y$\\$y\Uparrow x$} & \tabincell{c}{$x\nparallel y$\\$y\nparallel x$}\\ \hline
	\end{tabular}
\end{table}
...
\caption{A summary of qualitative comparison of RORU vs. ExRORU\label{tab:summary}}
	\begin{tabular}{|l|l|l|}\hline
		...
		Types of relations & \tabincell{l}{$\{\twoheadrightarrow,\rightharpoonup,\mapsto,\rightleftharpoons,$\\$~\equiv,\Downarrow,\Uparrow,\Updownarrow\}$} & \tabincell{l}{$\{\overset{D}{\twoheadrightarrow},\overset{I}{\twoheadrightarrow},\overset{D}{\rightharpoonup},\overset{I}{\rightharpoonup},\nrightarrow,\overset{D}{\twoheadleftarrow},\overset{I}{\twoheadleftarrow},$\\$~\overset{D}{\leftharpoonup},\overset{I}{\leftharpoonup},\nleftarrow,\Updownarrow,\Uparrow,\nparallel\}$}\\ \hline
